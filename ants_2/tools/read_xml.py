...
def get_staxml(net,sta):
    client=fdsn.Client()
    outfile='meta/stationxml/'+net+'.'+sta+'.xml'
    print outfile
    # Metadata request with obspy
    if os.path.exists(outfile)==False:
        client.get_stations(network=net,station=sta,filename=outfile,level='channel')
...
def get_coord_staxml(net1, sta1):
    ...
    try:
        (staname1,lat1,lon1)=find_coord('meta/stationxml/'+net1+'.'+sta1+'.xml')
